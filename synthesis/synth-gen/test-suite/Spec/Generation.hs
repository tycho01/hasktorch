...
gen = parallel $ let
    ...
    it "fnOutputs" $ do
        ...
        -- not
        hm1 <- interpretUnsafe $ fnOutputs crashOnError maxParamsDef (singleton bl [con "True", con "False"]) (var "not") [([bl], bl)]
        pp_ hm1 `shouldBe` pp_ ((singleton (bl, bl) [(paren (parseExpr "True"), Right (parseExpr "False")), (paren (parseExpr "False"), Right (parseExpr "True"))]) :: HashMap (Tp, Tp) [(Expr, Either String Expr)])
        -- (+)
        hm2 <- interpretUnsafe $ fnOutputs crashOnError maxParamsDef (singleton int_ (int <$> [1,2,3])) (parseExpr "(+)") [([int_,int_], int_)]
