...
decayWeights' v parameters (D.Gradients gradients) = D.Gradients $ zipWith (\ param gradient -> (if v == 0.0 then id else F.add (F.mulScalar v param)) gradient) (toDependent <$> parameters) gradients
