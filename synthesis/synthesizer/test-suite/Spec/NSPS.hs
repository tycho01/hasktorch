...
nsps = parallel $ let
    ...
    it "LstmEncoder" $ do
        ...
        enc_model :: LstmEncoder Device MaxStringLength EncoderBatch' EncoderChars H FeatMultWithoutTypes <- A.sample $ LstmEncoderSpec charMap $ LSTMSpec $ DropoutSpec dropOut
        (_gen, io_feats) :: (StdGen, Tensor Device 'D.Float '[R3nnBatch', 2 * FeatMultWithoutTypes * Dirs * H * MaxStringLength]) <- sampleTensorWithoutReplacement @0 @R3nnBatch' gen (length io_pairs) . toDynamic $ lstmEncoder enc_model tp_io_pairs
        ...
        let enc_model' :: LstmEncoder Device MaxStringLength EncoderBatch' EncoderChars H FeatMultWithoutTypes = A.replaceParameters enc_model newParam
    ...
    it "LstmEncoder: typed" $ do
        -- say_ . show $ size charMap + 1
        enc_model :: LstmEncoder Device MaxStringLength EncoderBatch' EncoderChars H FeatMultWithTypes <- A.sample $ LstmEncoderSpec charMap $ LSTMSpec $ DropoutSpec dropOut
        (_gen, io_feats) :: (StdGen, Tensor Device 'D.Float '[R3nnBatch', 2 * FeatMultWithTypes * Dirs * H * MaxStringLength]) <- sampleTensorWithoutReplacement @0 @R3nnBatch' gen (length io_pairs) . toDynamic $ lstmEncoder enc_model tp_io_pairs
        D.shape (toDynamic io_feats) `shouldBe` [natValI @R3nnBatch', natValI @(2 * FeatMultWithTypes * Dirs * H * MaxStringLength)]

        let optim :: D.Adam = d_mkAdam 0 0.9 0.999 $ A.flattenParameters enc_model
        let loss :: Tensor Device 'D.Float '[] = sumAll io_feats  -- dummy op for loss with gradient
        (newParam, optim') <- D.runStep enc_model optim (toDynamic loss) lr
        let enc_model' :: LstmEncoder Device MaxStringLength EncoderBatch' EncoderChars H FeatMultWithTypes = A.replaceParameters enc_model newParam
    ...
    it "TypeEncoder" $ do
        ...
        rule_tp_emb :: Tensor Device 'D.Float '[Rules, MaxStringLength * M] <-
    ...
    it "R3NN" $ do
        ...
        enc_model :: LstmEncoder Device MaxStringLength EncoderBatch' EncoderChars H FeatMultWithoutTypes <- A.sample $ LstmEncoderSpec charMap $ LSTMSpec $ DropoutSpec dropOut
        (_gen, sampled_feats) :: (StdGen, Tensor Device 'D.Float '[R3nnBatch', MaxStringLength * (2 * FeatMultWithoutTypes * Dirs * H)])
                <- sampleTensorWithoutReplacement @0 @R3nnBatch' gen (length io_pairs) . toDynamic $ lstmEncoder enc_model tp_io_pairs
        r3nn_model :: R3NN Device M Symbols Rules MaxStringLength R3nnBatch' H TypeEncoderChars FeatMultWithoutTypes <- A.sample $ initR3nn variants r3nnBatch' dropOut ruleCharMap
        ...
        rule_tp_emb :: Tensor Device 'D.Float '[Rules, MaxStringLength * M] <-
                typeEncoder @Rules rule_encoder variantTypes
        hole_expansion_probs :: Tensor Device 'D.Float '[NumHoles, Rules] <- runR3nn r3nn_model symbolIdxs ppt rule_tp_emb sampled_feats
        ...
        let r3nn_model' :: R3NN Device M Symbols Rules MaxStringLength R3nnBatch' H TypeEncoderChars FeatMultWithoutTypes = A.replaceParameters r3nn_model newParam
        hole_expansion_probs' :: Tensor Device 'D.Float '[NumHoles, Rules] <- runR3nn r3nn_model' symbolIdxs ppt rule_tp_emb sampled_feats
    ...
    it "R3NN: typed" $ do
        let variant_sizes :: HashMap String Int = fromList $ variantInt . snd <$> variants
        enc_model :: LstmEncoder Device MaxStringLength EncoderBatch' EncoderChars H FeatMultWithTypes <- A.sample $ LstmEncoderSpec charMap $ LSTMSpec $ DropoutSpec dropOut
        (_gen, sampled_feats) :: (StdGen, Tensor Device 'D.Float '[R3nnBatch', MaxStringLength * (2 * FeatMultWithTypes * Dirs * H)])
                <- sampleTensorWithoutReplacement @0 @R3nnBatch' gen (length io_pairs) . toDynamic $ lstmEncoder enc_model tp_io_pairs
        r3nn_model :: R3NN Device M Symbols Rules MaxStringLength R3nnBatch' H TypeEncoderChars FeatMultWithTypes <- A.sample $ initR3nn variants r3nnBatch' dropOut ruleCharMap
        let symbolIdxs :: HashMap String Int = indexList $ "undefined" : keys dsl
        let ppt :: Expr = parseExpr "not (not (undefined :: Bool))"
        rule_encoder <- A.sample type_encoder_spec
        -- say_ $ "rule_tp_emb : " <> show rule_tp_emb
        ...
        let loss :: Tensor Device 'D.Float '[] = patchR3nnLoss r3nn_model variant_sizes $ sumAll hole_expansion_probs  -- dummy op for loss with gradient
        (newParam, optim') <- D.runStep r3nn_model optim (toDynamic loss) lr
        let r3nn_model' :: R3NN Device M Symbols Rules MaxStringLength R3nnBatch' H TypeEncoderChars FeatMultWithTypes = A.replaceParameters r3nn_model newParam
        hole_expansion_probs' :: Tensor Device 'D.Float '[NumHoles, Rules] <- runR3nn r3nn_model' symbolIdxs ppt rule_tp_emb sampled_feats
        let loss' :: Tensor Device 'D.Float '[] = patchR3nnLoss r3nn_model' variant_sizes $ sumAll hole_expansion_probs'
        toBool (loss' <. loss) `shouldBe` True
    ...
    it "calcLoss" $ do
        ...
        let encoder_spec :: LstmEncoderSpec Device MaxStringLength EncoderBatch' EncoderChars H FeatMultWithTypes =
        ...
        let r3nn_spec :: R3NNSpec Device M Symbols Rules MaxStringLength R3nnBatch' H TypeEncoderChars FeatMultWithTypes =
        ...
        let maskBad = False
        rule_tp_emb :: Tensor Device 'D.Float '[Rules, MaxStringLength * M] <-
                liftIO $ rule_encode @Device @'[R3nnBatch', MaxStringLength * (2 * FeatMultWithTypes * Dirs * H)] @Rules @(MaxStringLength * M) model variantTypes
        ...
        let model' :: NSPS Device M Symbols Rules MaxStringLength EncoderBatch' R3nnBatch' EncoderChars TypeEncoderChars H FeatMultWithTypes = A.replaceParameters model newParam
        loss' :: Tensor Device 'D.Float '[] <- interpretUnsafe $ calcLoss @Rules randomHole dsl task_fn taskType symbolIdxs model' sampled_feats variantMap ruleIdxs variant_sizes synth_max_holes maskBad variants rule_tp_emb
        toBool (loss' <. loss) `shouldBe` True
